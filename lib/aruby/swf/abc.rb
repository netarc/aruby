module ARuby
  class SWF
    ...
    #===============================================================================
      ...
      # Given a Core::Class object, create our ABC Class
      def create_class(klass)
        unless klass.is_a?(ARuby::Core::Class)
          raise StandardError, "create_class can only accept ARuby::Core:Class objects"
        end

        puts "create_class #{klass.name}|#{klass.super_name}"
        class_index, cls, inst, is_new = create_or_find_class(klass.name, klass.super_name)
        ...
        klass.methods[:instance][:private].each do |kmethod|
          ...
          # name.match(/^(?:\w+\/)?(\w+)$/)
          # mn = get_qname($1, "", ARuby::ABC::Namespace::PackageNamespace)
          #
          # trait = ARuby::ABC::Trait.new(self)
          # trait.name_index = mn
          # trait.type = ARuby::ABC::Trait::MethodId
          # trait.data = ARuby::ABC::TraitMethod.new(self)
        ...
        klass.methods[:instance][:public].each do |kmethod_key, kmethod|
          ...
          if kmethod_key == :"initialize"
            ...
            method_inst_body.code.codes << ARuby::ABC::OpCode::ReturnVoid.new(self)
            ...
            method_inst_body.code.codes.insert(-1, *kmethod.codes)
  #          method_inst_body.code.codes << ARuby::ABC::OpCode::ReturnVoid.new(self)
            method_inst_body.code.codes << ARuby::ABC::OpCode::ReturnValue.new(self)
          end

          method_inst_body.code.codes.insert(0, *[ARuby::ABC::OpCode::DebugFile.new(self, :index => @constant_pool.add_string("/Users/jhollenbeck/Projects/aruby/test;;Basic.as")), ARuby::ABC::OpCode::DebugLine.new(self, :value => method_inst_body.method_index)])
    ...
    private
      ...
      def redefine_abc(codes)
        codes.each do |code|
          if code.is_a? ARuby::ABC::OpCode::FindPropStrict
            code.index = get_qname(code, "", ARuby::ABC::Namespace::PackageNamespace)
          end
        end
      end
      ...
      def create_method
        # Create Method
        method = ARuby::ABC::Method.new(self)
        method_index = @abc_methods.length
        @abc_methods << method
        # Create Method Body
        method_body = ARuby::ABC::MethodBody.new(self)
      ...
      def create_class_init_method(cls)
        ...
        codes << ARuby::ABC::OpCode::GetLocal0.new(self)
        codes << ARuby::ABC::OpCode::PushScope.new(self)
        codes << ARuby::ABC::OpCode::ReturnVoid.new(self)
      end
      ...
      def create_class_constructor_stub
        ...
        codes << ARuby::ABC::OpCode::PushScope.new(self)
        codes << ARuby::ABC::OpCode::GetLocal0.new(self)
        codes << ARuby::ABC::OpCode::ConstructSuper.new(self, :arg_count => 0)
        codes << ARuby::ABC::OpCode::ReturnVoid.new(self)
        method_inst_body.max_stack = 1
        method_inst_body.local_count = 1
      ...
      def create_or_find_class(fullname, super_fullname="")
        ...
        # Namespaces
        class_ns = get_qname(class_name, class_package_name, ARuby::ABC::Namespace::PackageNamespace)
        super_class_ns = get_qname(super_class_name, super_class_package_name, ARuby::ABC::Namespace::PackageNamespace)
        ...
        cls = ARuby::ABC::Class.new(self)
        classes << cls

        inst = ARuby::ABC::Instance.new(self)
        instances << inst

        inst.name_index = class_ns
        inst.super_name_index = super_class_ns
        inst.flags = ARuby::ABC::Instance::ProtectedNamespace
        inst.protected_namespace_index = find_or_create_namespace(protected_namespace_name(class_name, class_package_name), ARuby::ABC::Namespace::ProtectedNamespace)
       ...
       def create_class_init_method_body(inst, class_index, mbody)
          ...
          hierarchy.each do |clz_sym|
            ...
            ns = get_qname(name, package, ARuby::ABC::Namespace::PackageNamespace)
            codes << ARuby::ABC::OpCode::GetLex.new(self, :index => ns)
            codes << ARuby::ABC::OpCode::PushScope.new(self)
          end
          ...
          codes << ARuby::ABC::OpCode::NewClass.new(self, :index => class_index)

          hierarchy.each do |clz|
            codes << ARuby::ABC::OpCode::PopScope.new(self)
          end

          codes << ARuby::ABC::OpCode::InitProperty.new(self, :index => inst.name_index)
          codes << ARuby::ABC::OpCode::ReturnVoid.new(self)
        end
        ...
        def create_doc_class_init_script(inst, class_index, declared_on_line)
          ...
          create_class_init_method_body(inst, class_index, scr.init.body) do |codes|
            codes << ARuby::ABC::OpCode::DebugFile.new(self, :index => @constant_pool.add_string("/Users/jhollenbeck/Projects/aruby;;example.rb"))
            codes << ARuby::ABC::OpCode::DebugLine.new(self, :value => declared_on_line)

            codes << ARuby::ABC::OpCode::GetScopeObject.new(self, :index => 0)
          end
        end
        ...
        def shared_class_script_init(inst, class_index)
          ...
          trait.type = ARuby::ABC::Trait::ClassId
          trait.data = ARuby::ABC::TraitClass.new(self)
      ...
      def find_or_create_namespace(name, type)
        ...
        @constant_pool.add_namespace(ARuby::ABC::Namespace.new(type, name_index, @constant_pool))
      end
      ...
      def get_qname(name, namespace, ns_kind=nil)
        ...
        name_index = @constant_pool.add_string(name)
        mn_index = @constant_pool.add_multiname(ARuby::ABC::Multiname.new(ARuby::ABC::Multiname::MultinameQName, name_index, ns_index, nil, @constant_pool))
      end
    end
  end
end
